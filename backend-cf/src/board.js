// SessionBell usage board — shared by the Worker route (/board) and the
// local script (scripts/board.mjs). Pure: takes a query function and an ASC
// snapshot, returns HTML. Only counts, timestamps and 6-char account ids
// reach the page — never `detail`, `capture` or other users' host names.

import { countryTotals, APP_ID as APP_ID_LABEL } from './asc.js';

/**
 * @param {object} o
 * @param {(sql: string) => Promise<any[]>} o.db   run a read-only SQL statement
 * @param {object} o.asc                          result of asc.snapshot()
 * @param {'live'|'file'} [o.mode]                affects the footer note only
 * @param {number} [o.ascTtlMin]                  shown in the live footer
 * @param {number} [o.now]
 */
export async function renderBoard({ db, asc, mode = 'file', ascTtlMin = 30, now = Date.now() }) {
const DAY = 86400e3;
...
const stateRows = await db(`SELECT ns, v, ts FROM kv WHERE k LIKE 'state/%'`);
...
const feedbackRows = await db(`SELECT k, v, ts FROM kv WHERE ns='sys' AND k>='feedback/' AND k<'feedback/\uffff' ORDER BY ts DESC LIMIT 30`);
  ...
  if (!asc.configured) {
    return `<div class="panel setup">
      ...
      <ol>
        ...
        <li>本地脚本:把 Key ID、Issuer ID、.p8 路径写到 <code>~/.sessionbell/asc.json</code>;线上 Worker:<code>wrangler secret put ASC_KEY &lt; AuthKey.p8</code>,并在 wrangler.jsonc 的 vars 里填 ASC_KEY_ID / ASC_ISSUER_ID / ASC_VENDOR。</li>
